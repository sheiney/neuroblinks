...
%         stim=zeros(m,n,3,t,'uint8');
    case 'puff'
        stfrm{1}=round(metadata.cam(1).time(1)./1000.*metadata.cam(1).frame_rate);
        enfrm{1}=round(metadata.stim.totaltime./1000.*metadata.cam(1).frame_rate);
%         stim=zeros(m,n,3,t,'uint8');
...
%         stim(400:449,450:499,2,:)=255;
        cchan(1)=2;
    case 'electrical'       
        stfrm{1}=round((metadata.cam(1).time(1)+metadata.stim.e.delay)./1000.*metadata.cam(1).frame_rate);
        enfrm{1}=round(metadata.stim.e.traindur./1000.*metadata.cam(1).frame_rate);
        cchan(1)=1;
%         stim=zeros(m,n,3,t,'uint8');
%         stim=zeros(m,n,3,enfrm-stfrm,'uint8');
...
%         stim(400:449,500:549,1,:)=255;
    ...
    case {'conditioning','electrocondition'}
        stfrm{1}=round(metadata.cam(1).time(1)./1000.*metadata.cam(1).frame_rate); % for CS
        % enfrm{1}=round(metadata.stim.c.csdur./1000.*metadata.cam(1).frame_rate);
        enfrm{1}=round((metadata.stim.c.isi+metadata.stim.c.usdur)./1000.*metadata.cam(1).frame_rate);    % Had to change this bc was getting error if CS was longer than recorded trial time
        cchan(1)=3;
        stfrm{2}=round((metadata.cam(1).time(1)+metadata.stim.c.isi)./1000.*metadata.cam(1).frame_rate);
        enfrm{2}=round(metadata.stim.c.usdur/1000.*metadata.cam(1).frame_rate); % for US
        cchan(2)=2;

    case 'optical'      
        stfrm{1}=round((metadata.cam(1).time(1)+metadata.stim.l.delay)./1000.*metadata.cam(1).frame_rate);
        enfrm{1}=round(metadata.stim.l.traindur./1000.*metadata.cam(1).frame_rate);
        cchan(1)=3;
%         stim=zeros(m,n,3,t,'uint8');
%         stim=zeros(m,n,3,enfrm-stfrm,'uint8');
