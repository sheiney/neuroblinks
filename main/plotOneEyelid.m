function plotOneEyelid(ax, t_num)
...
for i=1:length(trials.eye(t_num).stimtime.st)
    if trials.eye(t_num).stimtime.en{i}>0
        rectangle(ax, 'position',[trials.eye(t_num).stimtime.st{i} -0.15+0.04*(i-1) trials.eye(t_num).stimtime.en{i} 0.04],...
            'facecolor',stimcol{trials.eye(t_num).stimtime.cchan(i)},'edgecolor','none')
    end
end
% -- plot eye ----
plot(ax, trials.eye(t_num).time(1:ti_pre(end)+1),trials.eye(t_num).trace(1:ti_pre(end)+1),'k')
plot(ax, trials.eye(t_num).time(ti_post(1)-1:end),trials.eye(t_num).trace(ti_post(1)-1:end),'b')
plot(ax, trials.eye(t_num).time(ti_isi),trials.eye(t_num).trace(ti_isi),'r')
