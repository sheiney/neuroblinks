...
metadata.cam(1).ROIposition = cameras(1).ROIposition ./ config.camera(1).binning;
metadata.cam(1).fullsize = cameras(1).ROIposition ./ config.camera(1).binning;

cameras(2) = addCam(config.camera(2).IDS{config.rig}, config);
configureBaslerPulse(cameras(2), config);

metadata.cam(2).ROIposition = cameras(2).ROIposition ./ config.camera(2).binning;
metadata.cam(2).fullsize = cameras(2).ROIposition ./ config.camera(2).binning;
